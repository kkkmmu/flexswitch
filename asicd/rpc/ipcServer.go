...
func NewAsicDaemonServiceHandler(logger *logging.Writer) *AsicDaemonServiceHandler {
...
func NewAsicdServer(socket string, logger *logging.Writer) *AsicDaemonServerInfo {
	transport, err := thrift.NewTServerSocket(socket)
	if err != nil {
		panic(err)
	}
	handler := NewAsicDaemonServiceHandler(logger)
