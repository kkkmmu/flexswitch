...
func getClient(logger *logging.Writer, fileName string, process string) (*ClientJson, error) {
	...
	if err != nil {
		logger.Err(fmt.Sprintf("Failed to open STPD config file:%s, err:%s", fileName, err))
		return nil, err
	}
...
func StartServer(logger *logging.Writer, handler *LACPDServiceHandler, paramDir string) {
	...
	if err != nil {
		logger.Err(fmt.Sprintln("StartServer (lacpd): NewTServerSocket failed with error:", err))
		return
	}
